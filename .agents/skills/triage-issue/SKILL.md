...
4. Infer the most likely related files and estimate reproducibility as `high`, `medium`, `low`, or `unknown`. Tag regressions ("broke in recent version", "worked before"), crashes/panics/data loss, or security vulnerabilities with `priority:high`.
