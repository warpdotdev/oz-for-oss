...
Agent-backed work runs through a Vercel-hosted webhook control plane (`api/`, `lib/`, `tests/`, `vercel.json`). The only GitHub Actions workflow kept in this repository is CI in [`.github/workflows/run-tests.yml`](.github/workflows/run-tests.yml); bot delivery no longer depends on reusable Actions wrappers under `.github`.
